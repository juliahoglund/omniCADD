...
params: 
  ...
  ingroup: 'None'
  outgroup: 'None'
  path: '/Users/juliahoglund/Documents/omniCADD/'
...
list.of.packages <- c("ggfittext", "patchwork", "ggplotify", "gridExtra", "BiocManager", "kableExtra", "ape")
list.of.bioconductor <- c("biomaRt", "chromPlot", "GenomicFeatures", "BSgenome", "ggtree", "emojifont", "ggpubr")
...
# MAKE YES AUTOMATIC ON INSTALL FROM SOURCE, and update all!!
...
fraction.used <- 
  ...
  group_by(CHROM) %>%
  mutate(pct=n/sum(n),
         CHROM = factor(CHROM, levels = c(1:19, 'X'))) %>%    
  ggplot(aes(CHROM, n, fill=FORMAT)) +
...
mutation.types <- 
  ...
  mutate(pct=n/sum(n),
         CHROM = factor(CHROM, levels = c(1:19, 'X'))) %>%    
  ggplot(aes(CHROM, n, fill=FILTER)) +
  geom_bar(stat="identity") +
  ggfittext::geom_bar_text(aes(label=paste0(sprintf("%1.1f", pct*100),"%")), color = "#333e3d", position = "stack", reflow = TRUE) +
  labs(y= "count", x = "chromosome") + 
