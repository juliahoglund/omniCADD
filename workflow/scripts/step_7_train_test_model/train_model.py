...
if __name__ == '__main__':
    try:
        ...
        x_train, y_train, columns = load_dataset(args.train, args.columns, jobs=args.n_jobs)
        ...
        logging.info("Saving model(s) to disk")
        ...
        x_test, y_test, columns = load_dataset(args.test, args.columns, jobs=args.n_jobs)
        x_test = scaler.transform(x_test)

        for model, (c, max_iter) in zip(models, params):
            logging.info(f"Saving tests for model c={c}, max_iter={max_iter}")
            ...
            result = pandas.concat({"y_true": pandas.Series(y_test),
                                   ...
                                   axis=1)
            result.to_csv(get_file(c, max_iter) + ".pred.csv.gz", index=False, float_format="%.10g")
            ...
            with open(get_file(c, max_iter) + ".stats.txt", "w") as stat_f:
                ...
                stat_f.write("\nStandard scikit-learn classification report output:\n")
