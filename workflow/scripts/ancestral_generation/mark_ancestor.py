...
def main(args):
    ...
    try:
        input_file = gzip.open(args.input, "rt") if args.input.endswith('.gz') else open(args.input, "r")
        with input_file as alignment_file:
            output_file = (
                gzip.open(args.output, "wt", compresslevel=1)
