...
# Function to read and label SNP files
read_snp_file <- function(file, label) {
  snp_data <- read.table(file, header = F)
  colnames(snp_data) <- c("CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO")
  return(snp_data)
}
...
tryCatch({
  ...
  simulatedSNPs <- read_snp_file(opt$snp, "simulatedSNPs")
  simulatedAncestorSNPs <- read_snp_file(opt$snpFiltered, "simulatedAncestorSNPs")

  message("Reading files with derived variants ...")

  derivedSNPs <- read_snp_file(opt$derived, "derivedSNPs")
