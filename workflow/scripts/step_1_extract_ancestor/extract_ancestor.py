...
parser.add_argument("-i", "--input",
				...
				"it has been filtered and sorted.", 
				required = True)
parser.add_argument("-o", "--output", 
				help = "Ancestral sequence output file, fasta format", required = True)
...
def sequence_processing(g_seq, ancestor_seq):
	...
	if len(anc_out_seq) != anc_out_seq.annotations['size']:
		sys.stderr.write(
			'The length of the processed ancestor sequence differs from the stated size: '
...
# check the position from where it is taken etc etc
	...
	for alignment in alignment_gen:
		...
		if (not isinstance(anc_seq, SeqIO.SeqRecord)) or (str(intrest_n) + '.' not in sp_seq.id):
			if (isinstance(anc_seq, SeqIO.SeqRecord)) and (str(intrest_n) + '.' not in sp_seq.id):
				sys.exit(
					'In the preprocessing is something wrong. The alignment block contains an ancestor sequence but not ' +
...
for i,position in enumerate(list(pos_list_s)[1:]):
	...
	if anc_pos_seq_dict[position].annotations['start'] < len(ancestor_record):
		...
		continue # not in original if use code here below
...
#			ancestor_record.seq = current_fasta
	...
	else: # use continue if above is taken away, indent below if else is used
...
ancestor_record.seq = ancestor_record.seq + Seq('-' * (
	anc_pos_seq_dict[position].annotations['srcSize'] - len(ancestor_record.seq)))
