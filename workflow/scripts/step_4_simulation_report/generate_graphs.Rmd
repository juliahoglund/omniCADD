...
if (params$ideogram == 'None') {
  ...
  # making the data frame for karyotype visualisation
  blobby <- data.frame(chrom = info.fai$chromosome, # keep the factors, one for each chrom
...
if (params$annotation == "None") {
  ideogram(karyotype = karyogram,
          ...
          # CHANGE HERE TO VISUALISATION
  
  suppressMessages(anc_seq <- svgparser::read_svg(paste("~/anc-seq.svg", sep = "")))
...
if (params$coverage == "None") {
  ...
  suppressMessages(anc_cov <- svgparser::read_svg(paste("~/anc-coverage.svg", sep = "")))
