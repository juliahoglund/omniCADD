...
parser.add_option("-l","--liftover", dest="liftover", help="Path (and name of) lift-over file", default="annotations/hg19ToSusScr11.over.chain.gz")
...
# 3 zip
# 4. remove if yes 
filenames = [x for x in os.listdir(options.phast) if x.endswith('txt.gz')]

for file in filenames:
	os.system('gunzip ' + str(options.phast) + str(file))
	file = file.replace('.gz', '')
	chrom = file.split('_')[0]
	os.system('liftOver ' + file + ' ' + options.liftover + ' ' + str(file.replace('.txt', '_lifted.tsv')) + ' unMapped_' + str(chrom))
...
if options.tidy=='yes':
	...
	os.system('rm ' + str(options.phast) + '*_phyloP_scores.txt.gz')
