...
rule combine_constraint:
    ...
    shell:
        """
        mkdir -p $(dirname {output.main}) results/temp logs/benchmarks
        Rscript {input.script} -c {wildcards.chr} -n {params.n_chunks} -f {input.phastcons} -g {input.phylop} -i {input.gerp} -j {input.index} 2>> {log}
...
rule intersect_bed:
    ...
    shell:
        ...
        python3 {input.script} -v {input.vep} -b {input.bed} -o /dev/stdout | gzip > {output} 2> {log}
        """
